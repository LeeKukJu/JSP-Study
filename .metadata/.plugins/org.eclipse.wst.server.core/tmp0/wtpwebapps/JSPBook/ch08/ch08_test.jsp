...
$(function(){
	...
	$("#quest2").focusout(function() {
		if(!regExpId.test($("#quest2").val())){
			$("#err_quest2").html("시작은 영소문자 a,b,c고 영소대문자 8-12자리까지만 입력 받을 수 있게 설정");
	...
	$("#mem_pw").focusout(function() {
		if(!regExpName.test($("#mem_pw").val())){
			$("#err_pw").html("비밀번호는 영대문자로 시작하고 영문 대 소문자, 숫자, 특수문자  8~16자로 설정");
	...
	$("#mem_name").focusout(function() {
		if(!regExpPhone.test($("#mem_name").val())){
	...
	$("#mem_phone").focusout(function() {
		if(!regExpQuest1.test($("#mem_phone").val())){
			$("#err_phone").html("핸드폰 번호는 연락처 형식에 맞춰서 설정!");
